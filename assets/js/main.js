...
// Get the tag data
function createMarquee(marqueeData) {
  var marqueeTemplate = [
    '<li>',
    marqueeData.Transcription,
    '<li>'
  ];

  // a jQuery node
  return $(marqueeTemplate.join(''));
}

var marquees = $();
// Store all the marquee nodes
marqueeData.forEach(function(marqueeItem, i) {
  marquees = marquees.add(createMarquee(marqueeItem));
});

// Add them to the page
$(function() {
  $('.marquee__content').append(marquees);
